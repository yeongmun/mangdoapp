...
3. 아래 도구막대에서 **손상 유형**을 고른 뒤 도면에 표기합니다
   ...
   - 철근노출은 드래그한 **위치와 방향만** 쓰고 사각형은 범례 크기(1341×332mm, 기호보다 조금 큼)로 고정됩니다. 크기가 필요하면 속성창에 가로/세로를 적으세요
   ...
   - 손상을 그리면 **바로 선택되고 속성창이 열립니다**(펜·손가락 그리기 모두). 값을 적고 저장하거나 닫으면 다음 손상을 그릴 수 있습니다. 복제·이동·탭에는 열리지 않습니다
   ...
   - 사진번호 칸 옆 **📷** 버튼을 누르면 카메라가 열리고, 찍은 사진은 **기기 앨범에 저장됩니다(서버에는 올리지 않습니다)**. 저장된 사진의 번호가 사진번호 칸에 자동으로 이어 붙으니, 나중에 앨범에서 그 번호로 찾으면 됩니다 — 아이폰/아이패드는 앨범 파일명의 `IMG_` 뒤 숫자, 안드로이드(갤럭시탭)는 `_` 뒤 6자리(찍은 시각)가 번호입니다. **이 기능을 쓰려면 앱을 완전히 껐다 켜야 합니다**(새 네이티브 모듈이 추가되었습니다 — Expo Go는 다시 열기만 하면 됩니다)
...
- **저장 실패 배지에 `schemaVersion은 4이어야 합니다`가 뜨면**: 앱(뷰어)은 새 코드인데 서버는 옛 코드로 돌고 있는 것입니다(2026-09-16 복제 기능부터 저장 형식이 5 — 옛 서버는 4를 기대해 새 문서를 거부합니다). `git pull` 뒤 `npm run server`를 다시 시작하세요. 반대로 `schemaVersion은 5이어야 합니다`가 뜨면 서버는 새 코드인데 뷰어가 옛 페이지를 캐시한 것이니 앱을 완전히 껐다 켜세요. 예전 도면의 손상 기록은 열 때 저절로 새 형식으로 바뀝니다.
