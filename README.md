...
- **저장 실패 배지에 `schemaVersion은 5이어야 합니다`가 뜨면**: 앱 코드를 새로 받았는데 서버는 옛 코드로 돌고 있는 것입니다(2026-09-16 복제 기능부터 저장 형식이 5). `git pull` 뒤 `npm run server`를 다시 시작하세요. 예전 도면의 손상 기록은 열 때 저절로 새 형식으로 바뀝니다.
