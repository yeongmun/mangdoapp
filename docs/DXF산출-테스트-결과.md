# DXF 산출 확인 결과

- 확인 일자:
- 커밋:
- 확인 도면:
- 캐드: 지스타캐드 (버전:            )

| # | 확인 항목 | 결과 | 비고 |
|---|---|---|---|
| 1 | 내려받은 DXF가 지스타캐드에서 오류 없이 열린다 | | | 오류없이 열린다
| 2 | 원본 도면이 그대로 있다 (선·글자·표가 하나도 사라지거나 바뀌지 않았다) | | | 그대로 있다
| 3 | 손상이 빨간색 `신규손상` 레이어로 올라가 있다 (레이어 끄면 손상만 사라진다) | | |
| 4 | 손상 위치가 앱 화면에서 본 자리와 같다 | | |
| 5 | 균열은 열린 선, 면형은 닫힌 사각형으로 들어갔다 | | |
| 6 | 해치 무늬가 사내 망도와 **같은 간격**이다 (박리 ANSI31, 박락 ANSI37, 재료분리 CORK, 백태·열화 TRIANG, 기타·파손 ANSI33, 망상균열 ANCHORLK) | | |
| 7 | 망상균열의 `ANCHORLK` 무늬가 사내 망도와 똑같이 보인다 (`.pat` 파일 없이도) | | |
| 8 | 철근노출 기호가 나란한 선 2개 + 양 끝 ✕ 모양이다 (✕는 사각형 **짧은 변**보다 크면 그 변에 맞춰 줄어들어 범례와 크기가 다를 수 있다 — 사각형을 벗어나지 않는다. 선은 ✕ 중심이 아니라 ✕ 대각선과 만나는 자리에서 멈춰 ✕를 뚫지 않는다) | | |
| 9 | 균열/백태의 원이 선을 따라 위·아래 번갈아 찍혀 있다 | | |
| 10 | 라벨의 번호 원과 글자 크기가 도면의 다른 글자와 어울린다 (글자 높이 300) | | |
| 11 | 라벨 줄 순서가 앱 화면과 같다 (번호+이름 / 치수 / 사진번호) | | |
| 12 | 라벨의 한글이 깨지지 않는다 | | |
| 13 | 물량표의 각 칸에 값이 들어가고 번호 순서가 앱과 같다 | | |
| 14 | 값이 없는 칸은 비어 있다 (`-`가 찍히지 않는다) | | |
| 15 | 번호 칸과 손상위치 칸은 손대지 않았다 | | |
| 16 | 물량표의 한글(손상현황)이 깨지지 않는다 | | |
| 17 | 표 글자가 칸 가운데에 놓여 있다 (칸을 벗어나지 않는다) | | |
| 18 | **한 망도틀에 손상을 35개를 넘게(36개부터) 그린 도면**에서 그 틀이 **오른쪽에 통째로 복사**된다 (테두리·표·구조물 도면까지 똑같이) | | |
| 19 | 복사된 틀에도 손상 도형·라벨이 **1번부터 전부** 그려져 있고, 표만 **36~70**으로 이어진다 | | |
| 20 | 복사된 틀 **뒤에 있던 망도틀들이 한 틀씩 오른쪽으로 밀렸다** (구조물 도면·도면 번호·표가 함께 이동) | | |
| 21 | 지스타캐드에서 저장한 뒤 다시 열어도 손상이 그대로 있다 (왕복) | | |
| 22 | 지스타캐드에서 DWG로 저장한 뒤 다시 열어도 손상이 그대로 있다 | | |
| 23 | 손상을 하나도 표기하지 않은 도면에서 내려받으면 `표기한 손상이 없습니다`가 뜬다 | | |
| 24 | DWG로 올린 도면 줄에는 내려받기 버튼 대신 `DXF로 올린 도면만`이 보인다 | | |
| 25 | 손상현황을 띄어쓰기 포함해서 적은 **기타** 손상(예: `표면 오염 심함`)도 물량표 칸에서 줄바꿈 없이 한 줄로 보인다 | | |
| 26 | 라벨 첫 줄이 번호 원 **옆**에 붙어 있다 (균열은 치수 `⑰ 0.2/0.3`, 다른 유형은 이름 `⑰ 파손`) | | |
| 27 | 둘째 줄부터 첫 줄 글자의 왼쪽에 맞춰 들여쓰기가 되어 있다 (빈 줄이 없다) | | |
| 28 | 사진번호가 `#12, #13` 형식이다 (`사진 ` 접두어가 없다) | | |
| 29 | 사진번호가 **노란색**이고 `사진번호` 레이어에 있다 (그 레이어만 끄면 사진번호만 사라진다) | | |
| 30 | 붙어 있는 손상들의 라벨이 서로 겹치지 않는다 | | |
| 31 | 자리를 옮긴 라벨 중 **화살대가 화살촉 길이(글자 높이의 절반, 150mm)보다 긴 것**에는 화살표가 있고 그 손상을 가리킨다 (화살촉이 손상 경계에 닿는다). 손상 바로 옆·아래(간격 100)로만 살짝 밀린 라벨에는 화살표가 없다 | | |
| 32 | 앱 화면에서 본 라벨 자리·화살표와 내려받은 DXF가 **같다** | | |
| 33 | 망도틀마다 번호가 **1번부터** 다시 시작한다 (첫 틀이 15번에서 끝나도 다음 틀은 1번) | | |
| 34 | 틀마다 **자기 안의 물량표**에 그 틀 손상만 들어간다 (손상이 없는 틀의 표는 비어 있다) | | |
| 35 | 망도틀 밖에 그린 손상은 번호 원 없이 도형만 그려지고 `망도틀 밖 손상 N개…` 경고가 뜬다 | | |
| 36 | 앱 화면의 틀별 번호와 내려받은 DXF의 번호가 **같다** | | |
| 37 | 예: 001과 003이 둘 다 넘치면 복사본이 둘 생기므로 004 이후 틀은 **두 칸** 밀린다 | | |
| 38 | 복사된 틀의 표를 캐드에서 눌러 보면 **표 객체가 아니라 선·글자**다 (번호를 바꾸려고 펼쳐 넣었기 때문 — 원본 틀의 표는 그대로 표 객체다) | | |
| 39 | 복사된 틀의 해치가 원본과 **같은 무늬·간격**이다 (연관이 풀렸어도 모양이 같다) | | |
| 40 | `망도틀 N을(를) M장으로 나눴습니다` 경고가 뜨고 내려받기 자체는 성공한다 | | |
| 41 | 표 글자가 **손상물량표** 레이어에 있다 (그 레이어만 끄면 표에 채운 값만 사라지고 손상 도형은 남는다) | | |
| 42 | 열 순서가 다르거나 칸이 더 있는 표(예: 비고 칸이 더 있거나 순서가 바뀐 표)에도 값이 제자리에 채워진다 | | |
| 43 | 표 머리글을 읽을 수 없는 도면은 `물량표 머리글을 읽지 못해 열 순서를 기본값으로 채웠습니다` 경고가 뜨고 옛 고정 순서로 채워진다 — **구버전 템플릿**(표 블록에 가로/폭·세로/길이 같은 둘째 줄 머리글이 없는 것)은 이 경고가 항상 뜨며 정상이다. 값이 제 칸에 들어갔는지만 본다 | | |
| 44 | 복제한 손상의 도형이 **개수만큼** 그려져 있다 (3EA면 도형 3개) | | |
| 45 | 복제본에는 번호 라벨이 없고 **첫 도형에만** 번호 원·글자가 붙어 있다 | | |
| 46 | 물량표에 그 손상이 **한 행**만 있고 개소 칸이 도형 수와 같다 | | |
| 47 | 라벨이 복제본 도형 위에 얹히지 않았다 (겹치면 그 손상의 유형·위치를 적어 둘 것) | | |
| 48 | 갤럭시에서 속성창의 📷 → 찍기: 사진번호 칸에 `HHMMSS` 6자리가 붙고, 앨범에 `YYYYMMDD_HHMMSS.jpg`가 있다 | | |
| 49 | 아이패드에서 속성창의 📷 → 찍기: 사진번호 칸에 `IMG_` 뒤 숫자가 붙고, 사진 앱에서 그 사진의 정보를 열어 보면 같은 번호가 보인다 | | |
| 50 | 촬영 중 취소, 카메라·앨범 저장 권한 거부, PC 브라우저에서 📷를 눌렀을 때 각각 안내 문구만 뜨고 앱이 멈추거나 죽지 않는다 | | |
| 51 | 철근노출을 크게 드래그해도 사각형이 **1341×332mm**(기호보다 사방 60mm 큼)로 고정되고, 기호(✕–선–✕)가 그 안에 꽉 차 보인다. 화면과 DXF가 같다 | | |

## 라벨 겹침 어림값

설계 7장에서 미뤄 둔 항목이다. 글자 폭을 어림해서 잡으므로 아주 긴 이름(기타 손상현황)은 조금 겹칠 수 있다.

- 겹쳐 보이는 라벨이 있었는가(있다면 그 손상의 유형·손상현황):
- 화살표가 다른 손상을 가로질러 보기 나빴는가(사내 망도도 그렇게 그린다면 그대로 둔다):

## 넘침 장 복사

2026-09-16 설계에서 넘침 처리를 "표만 아래에 한 장 더"에서 **"틀을 통째로 오른쪽에 복사하고 뒤 틀을 민다"**로 바꿨다(사내 방식, 사용자 확인).

- 복사된 틀이 원래 뒤 틀이 있던 자리에 정확히 놓였는가(간격이 도면마다 조금씩 달라 어긋날 수 있다):
- 복사된 틀의 도면 번호 글자(`4-1` 등)가 원본과 같다 — 구분이 필요한가(필요하면 `4-1(2)` 같은 표기를 검토한다):
- 밀린 틀들 사이 간격이 원본과 같은가:
- 복사본에서 빠진 도형이 있었는가(경고에 개수가 나온다):

옛 방식(표를 원본 표 바로 아래에 한 장 더)은 **망도틀이 없는 도면**과 **회전·비균일 배율 틀**에만 남아 있다.

- 틀 없는 도면에서 두 번째 표가 아래에 생겼고 왼쪽 끝이 원본과 같은가:

## 발견한 문제

## 다음에 고칠 것
